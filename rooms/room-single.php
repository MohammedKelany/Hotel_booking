...
if (isset($_GET["room_id"])) {
    $utilities = $roomDB->get_room_utilities($_GET["room_id"]);
    $room = $roomDB->get_room_by_id($_GET["room_id"]);
    if (isset($_POST["submit"])) {
        if (!empty($_POST["name"]) && !empty($_POST["email"]) && !empty($_POST["phone"]) && !empty($_POST["check_in"]) && !empty($_POST["check_out"])) {
            $check_in = $_POST["check_in"];
            $check_out = $_POST["check_out"];
            if (date("Y/m/d") > $check_in  or date("Y/m/d") > $check_out) {
                echo "<script>alert('You should choose a valid check in date or check out date')</script>";
            } else if (date("Y/m/d") == $check_in or $check_out < $check_in) {
                echo "<script>alert('You should choose a valid check in that is not today or not greater than checkout date')</script>";
            } else {
                ...
                $email = $_POST["email"];
                $room_name = $room["name"];
                $num_of_nights = intval(date_diff(date_create($check_in), date_create($check_out))->format("%R%a"));
                $payment = $room["price"] * $num_of_nights;
                $_SESSION["price"] = $payment;
                $status = "Pending";
                $roomDB->booking_room($_GET["room_id"], $username, $email, $hotel_name, $user_id, $phone, $room_name, $payment, $status, $check_in, $check_out);
            }
        } else {
            echo "<script>alert('there are some empty values')</script>";
        }
    }
} else {
...
<section class="ftco-section ftco-book ftco-no-pt ftco-no-pb">
    <div class="container">
        <div class="row justify-content-end">
            <div class="col-lg-4">
                <form action="room-single.php?room_id=<?php echo $_GET["room_id"] ?>" method="post" class="appointment-form" style="margin-top: -568px;">
        ...
        <div class="row justify-content-center">
            <div class="col-md-9 text-center">
                ...
                <p class="mb-4">It’s safe to book online with us! Get your dream stay in clicks or drop us a line with
                    your questions.</p>
                <p class="mb-0"><a href="http://localhost/hotel-booking/about.php" class="btn btn-primary px-4 py-3">Learn More</a> <a href="http://localhost/hotel-booking/contact.php" class="btn btn-white px-4 py-3">Contact us</a>
                </p>
            </div>
        </div>
    </div>
